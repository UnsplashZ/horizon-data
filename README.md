...
前端（Vue+Vite）已可在 Mac 上独立验证：`npm install && npm run build`。
完整覆盖层需要 Rust（Tauri 后端）：

```bash
# 一次性：装 Rust（Mac/Windows 通用）
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh   # Windows 用 rustup-init.exe

npm install
npm run tauri dev      # 透明置顶覆盖窗口，内置监听 UDP:5300
npm run tauri build    # 出 .exe（Windows）：NSIS 安装版 + target/release 下的便携 exe
```

- 覆盖窗口默认**点击穿透**（不挡游戏），`set_click_through` 命令可切到可交互（编辑/拖动）。
- UDP 端口当前硬编码 5300（`src-tauri/src/main.rs` 的 `UDP_PORT`），后续做成可配置。
- Mac 远程调试：游戏 Data Out 指向 Mac 的局域网 IP:5300；窗口透明依赖已开启的 `macOSPrivateApi`。

## 路线图
1. ✅ UDP 接收+解析 demo（真机验证通过）
2. ✅ Tauri 透明置顶+点击穿透空覆盖窗口（Rust 收包 → emit 事件 → Vue 显示核心三件套）
3. 透明度滑块 + 拖动定位 + 端口可配置 + 配置持久化
